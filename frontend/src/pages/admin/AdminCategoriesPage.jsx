...
export default function AdminCategoriesPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast({
        title: error.response?.data?.message || "Failed to save category",
        variant: "destructive",
      });
    }
  };
  ...
  const flattenTree = (categories, depth = 0) => {
    if (!Array.isArray(categories)) return []; // ✅ fix
            ...
            <Table>
              ...
              <TableBody>
                {flatCategories.map((cat) => (
                  <TableRow
                    key={cat._id}
                    style={{ paddingLeft: cat.depth * 24 }}>
                    ...
                    <TableCell>
                      <span
                        className={`text-xs px-2 py-1 rounded ${cat.isActive ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"}`}>
                    ...
                    <TableCell className="text-right space-x-2">
                      ...
                      <Button
                        variant="ghost"
                        size="sm"
                        onClick={() => handleOpenDialog(null, cat._id)}>
                        Add Child
                      </Button>
                      <Button
                        variant="ghost"
